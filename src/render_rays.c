...
int	ft_get_wall_color(t_mlx *mlx, int orientation_flag)
{
	...
	if (orientation_flag == VERTICAL_WALL)
	{
		if (mlx->ray->ra > 90 && mlx->ray->ra < 270)
			return (0x0000FF00); // mur regarde  gauche
		else
			return (0x8B0000FF); // mur regarde a droite
	}
	else
	{
		if (mlx->ray->ra > 0 && mlx->ray->ra < 180)
			return (0xEBCE87FF); // mur regarde vers le bas
		else
			return (0xCD0000FF); // mur regarde vers le haut
	}
	return (0);
}
		...
		int wall_bot_px)
{
	...
	{
		ft_put_pixel_to_screen(mlx, ray_num, i, 0x006400FF);
		i++;
	}
	printf("Floor done\n");
	// CEILING
